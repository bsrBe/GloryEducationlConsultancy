...
export class StudentsController {
  ...
  findById(@Param('id') id: string, @Request() req: any) {
    ...
    if (req.user?.type === 'student' && currentId !== id) {
  ...
  verifyPayment(
    ...
    return this.studentsService.verifyPayment(
      id,
  ...
  async exportStudents(
    ...
    const csv = await this.studentsService.exportStudentsCSV({ search });
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=glory-students-export-${new Date().toISOString().split('T')[0]}.csv`,
    );
    return res.send(csv);
  }
