...
export class StudentsService {
  ...
  async verifyPayment(
    ...
    return {
      message: `Payment ${dto.status.toLowerCase()}`,
      payment: student.payments[paymentIndex],
    };
  }
  ...
  async assessStudent(
    ...
    const calculatedTotal =
      dto.totalScore ??
      Object.values(dto.categoryScores || {}).reduce(
  ...
  async overrideAssessment(
    ...
    if (!student.assessment)
      throw new BadRequestException('No assessment to override');
    student.assessment.totalScore = dto.score;
    student.assessment.override = {
      score: dto.score,
      reason: dto.reason,
      overriddenBy: new Types.ObjectId(overriddenBy),
  ...
  async suggestMatches(studentId: string, matchedBy: string) {
    ...
    const scored = universities.map((uni: any) => {
      ...
      const matchingProgram = programs.find(
        (p: any) =>
          p.name?.toLowerCase() === student.intendedProgram?.toLowerCase(),
      );
      if (matchingProgram) {
        score += 25;
        if (student.gpa >= (matchingProgram.gpaRequirement || 0)) score += 10;
      }
      if (student.englishScore) score += 10;
      return {
        university: uni,
        score,
        program: matchingProgram?.name || programs[0]?.name,
      };
    });
    ...
    const reasonBuilder = (match: any) => {
      const parts: string[] = [];
      if (student.preferredCountry)
        parts.push(`${student.preferredCountry} preference`);
      if (match?.program) parts.push(match.program);
      if (student.gpa) parts.push(`GPA ${student.gpa}`);
      if (student.englishTest && student.englishScore)
        parts.push(`${student.englishTest} ${student.englishScore}`);
      if (student.intake) parts.push(`${student.intake} intake`);
      return parts.join(' + ') || 'General match';
    };

    const matches = {
      primary: primary?.university
        ? {
            university: primary.university._id,
            program: primary.program,
            reason: reasonBuilder(primary),
          }
        : null,
      secondary: secondary?.university
        ? {
            university: secondary.university._id,
            program: secondary.program,
            reason: reasonBuilder(secondary),
          }
        : null,
      matchedBy: new Types.ObjectId(matchedBy),
      matchedAt: new Date(),
      status: 'suggested',
    };
  ...
  async approveMatch(studentId: string, dto: ApproveMatchDto) {
    ...
    if (!student) throw new NotFoundException('Student not found');
    if (!student.matches)
      throw new BadRequestException('No matches to approve');
    if (dto.primaryUniversityId)
      student.matches.primary.university = new Types.ObjectId(
        dto.primaryUniversityId,
      );
    if (dto.primaryProgram)
      student.matches.primary.program = dto.primaryProgram;
    if (dto.secondaryUniversityId)
      student.matches.secondary.university = new Types.ObjectId(
        dto.secondaryUniversityId,
      );
    if (dto.secondaryProgram)
      student.matches.secondary.program = dto.secondaryProgram;
    student.matches.status = dto.status || 'approved';
    await student.save();
  ...
  async submitReview(studentId: string, repId: string, dto: CreateReviewDto) {
    const student = await this.studentModel.findById(studentId);
    if (!student) throw new NotFoundException('Student not found');
    if (student.representativeReview?.isLocked)
      throw new BadRequestException('Review is already locked');
    student.representativeReview = {
      universityRep: new Types.ObjectId(repId),
    ...
    return {
      ...
      review: student.representativeReview,
    };
  }
  ...
  async publishResult(studentId: string, dto: PublishResultDto) {
    ...
    student.result = {
      ...
      disclaimer:
        dto.disclaimer ||
        "This assessment is preliminary. Final admission depends on the institution's official application and verification.",
    ...
    if (student.matches?.primary?.university) {
      const universityModel = this.studentModel.db.model('University');
      const uni = (await universityModel
        .findById(student.matches.primary.university)
        .lean()
        .catch(() => null)) as any;
  ...
  async getStudentResult(id: string) {
    const student = await this.studentModel
      .findById(id)
      .select(
        'studentId firstName lastName result matches representativeReview',
      )
      .lean();
    if (!student) throw new NotFoundException('Student not found');
    if (!student.result?.isPublished)
      return {
  ...
  async getAssignedStudents(repId: string) {
    return this.studentModel
      .find({
        'matches.primary.university': { $exists: true },
        $or: [
          { 'representativeReview.universityRep': new Types.ObjectId(repId) },
  ...
  async getStudentDashboard(studentId: string) {
    const student = await this.studentModel
      .findById(studentId)
      .select(
        'studentId firstName lastName email phone educationLevel school gpa graduationYear intendedProgram preferredCountry englishTest englishScore profileComplete payments documents assessment matches representativeReview result event application',
      )
      .lean();
    if (!student) throw new NotFoundException('Student not found');
    ...
    if (student.matches?.secondary?.university) {
      const universityModel = this.studentModel.db.model('University');
      const uni = (await universityModel
        .findById(student.matches.secondary.university)
        .lean()
        .catch(() => null)) as any;
    ...
    return {
      ...
      paymentStatus: student.payments?.some((p) => p.status === 'Verified')
        ? 'Verified'
        : 'Pending',
      documentStatus:
        student.documents?.length > 0
          ? student.documents[student.documents.length - 1].reviewStatus
          : 'Not Uploaded',
      assessmentStatus:
        student.assessment?.totalScore > 0 ? 'Completed' : 'Pending',
      ...
      representativeDecision:
        student.representativeReview?.decision || 'Pending',
  ...
  async exportStudents(query?: { search?: string }) {
    ...
    const students = await this.studentModel
      .find(filter)
      .select(
        'studentId firstName lastName email phone educationLevel gpa graduationYear intendedProgram preferredCountry englishTest englishScore profileComplete payments assessment matches representativeReview result application influencerSource createdAt',
      )
      .sort({ createdAt: -1 })
      .lean();
  ...
  async getDashboardAnalytics() {
    const [
      ...
      this.studentModel.countDocuments({
        'representativeReview.decision': { $exists: true },
      }),
      this.studentModel.countDocuments({
        'representativeReview.decision': 'Green',
      }),
      this.studentModel.countDocuments({
        'representativeReview.decision': 'Yellow',
      }),
      this.studentModel.countDocuments({
        'representativeReview.decision': 'Red',
      }),
      this.studentModel.countDocuments({ 'result.isPublished': true }),
      this.studentModel.aggregate([
        { $group: { _id: '$application.stage', count: { $sum: 1 } } },
      ]),
      this.studentModel.aggregate([
        { $group: { _id: '$influencerSource', count: { $sum: 1 } } },
      ]),
    ]);
    return {
      totalStudents,
      ...
      publishedResults,
      applicationStages: applicationStages.reduce((acc: any, item: any) => {
        acc[item._id || 'None'] = item.count;
        return acc;
      }, {}),
      influencerSources: influencerSources.reduce((acc: any, item: any) => {
        acc[item._id || 'organic'] = item.count;
        return acc;
      }, {}),
    };
  }
  ...
  async bulkEmail(dto: {
    ...
    if (dto.studentIds?.length) {
      ...
      switch (dto.filter) {
        case 'green':
          filter['representativeReview.decision'] = 'Green';
          break;
        case 'yellow':
          filter['representativeReview.decision'] = 'Yellow';
          break;
        case 'red':
          filter['representativeReview.decision'] = 'Red';
          break;
        case 'unpaid':
          filter.payments = { $not: { $elemMatch: { status: 'Verified' } } };
          break;
        case 'incomplete_profile':
          filter.profileComplete = { $lt: 100 };
          break;
        case 'all':
          break;
        default:
